...
class Rule extends EventEmitter implements ICouldHavePriority {
    ...
    evaluate (almanac: Almanac): Promise<RuleResult> {
        ...
        let evaluateConditions = (
          conditions: Condition[],
          arrConditionMethod: <T>(callbackfn: (value: T, index: number, array: T[]) => boolean, thisArg?: any) =>  boolean) => {
        ...
        if (ruleResult.conditions.any) {
            return anyOperation(ruleResult.conditions.any as Condition[])
                .then(result => processResult(result))
        } else {
            return allOperation(ruleResult.conditions.all as Condition[])
                .then(result => processResult(result))
        }
    }
}
