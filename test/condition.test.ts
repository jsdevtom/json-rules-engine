...
function condition (): { all: { id: string; fact: string; operator: string; value: number; path: string }[] } {
    return {
        all: [{
...
describe('Condition', () => {
    ...
    describe('evaluate', () => {
        ...
        describe('validations', () => {
            ...
            test('throws when missing an almanac', () => {
                return expect(condition.evaluate(undefined as any, operators)).rejects.toThrow('almanac required')
            })
            test('throws when missing operators', () => {
                return expect(condition.evaluate(almanac, undefined as any)).rejects.toThrow('operatorMap required')
            })
            test('throws when run against a boolean operator', () => {
                condition.all = []
                return expect(condition.evaluate(almanac, operators)).rejects.toThrow('Cannot evaluate() a boolean condition')
            })
        })
    ...
    describe('objects', () => {
        test(
            ...
            async () => {
                let condition = new Condition({operator: 'equal', path: '[0].id', fact: 'age', value: 50} as any)
    ...
    describe('atomic facts', () => {
        test('throws if no options are provided', () => {
            expect(() => new (Condition as any)()).toThrowError(/Condition: constructor options required/)
        })

        test('throws for a missing "operator"', () => {
            let conditions = condition()
            delete conditions.all[0].operator
            expect(() => new Condition(conditions)).toThrowError(/Condition: constructor "operator" property required/)
        })

        test('throws for a missing "fact"', () => {
            let conditions = condition()
            delete conditions.all[0].fact
            expect(() => new Condition(conditions)).toThrowError(/Condition: constructor "fact" property required/)
        })

        test('throws for a missing "value"', () => {
            let conditions = condition()
            delete conditions.all[0].value
            expect(() => new Condition(conditions)).toThrowError(/Condition: constructor "value" property required/)
        })
    })
